...
for(i in 1:40){ 
  for(j in 1:40){ if(dim(lookup_friendships(friends_and_followers[i], friends_and_followers[j]))[1] < 1){ 
    adj_val[i ,j] <- "FALSE" 
  }else{adj_val[i,j] <- lookup_friendships(friends_and_followers[i], friends_and_followers[j])[[11,4]]} 
  } 
}
adj_val
adj_valx <- adj_val 
...
ind_lb <- which(no_edges[, 3] == min(no_edges[, 3], na.rm = TRUE)) 
no_edges[ind_lb, ]
